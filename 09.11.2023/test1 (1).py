...
while True:
    ...
    hole_y = HOLES[5][1]-1
    # 5번홀과 흰공 거리 구하기
    a = math.sqrt(abs(hole_x- whiteBall_x)**2+abs(hole_y- whiteBall_y)**2)
    print(a)
    #5번홀과 타겟공 거리구하기
    b = math.sqrt(abs(hole_x- targetBall_x)**2+abs(hole_y- targetBall_y)**2)
    print(b)
    #흰공과 타겟공 거리구하기
    c = math.sqrt(abs(whiteBall_x- targetBall_x)**2+abs(whiteBall_y- targetBall_y)**2)
    print(c)
    # 각도 R(가)
    R_rad = math.atan(abs(hole_x-whiteBall_x)/abs(hole_y-whiteBall_y))
    R_deg = math.degrees(R_rad)

  
    # 각도 E (다)
    E_rad = math.acos(abs(a**2+b**2-c**2)/(2*a*b))
    E_deg = math.degrees(E_rad)


    # 흰공과 접공 거리구하기
    d = math.sqrt( a**2 + (b + 2*r)**2 - 2*a*( b + 2*r )*math.cos(E_rad))

    # 각도 S (나)
    S_rad = math.acos( (a**2 + d**2 - ( b + 2*r )**2) / (2*a*d) )
